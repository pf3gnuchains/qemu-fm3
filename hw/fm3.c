                     ...
                     const struct fm3_board_info_t *bi)
{
    ...
    nvic = armv7m_init(sysmem, bi->flash_size/1024, bi->sram_size/1024, 
                      bi->sram_start, kernel_filename, cpu_model);
    if (!nvic)
        return;
    ...
    for (n = 0; n < FM3_IRQ_NUM; n++) {
        sysbus_connect_irq(sysbus_from_qdev(dev), n, nvic[n]);
...
static void cq_fm3_init(ram_addr_t ram_size, const char *boot_device,
                        const char *kernel_filename, const char *kernel_cmdline,
                        const char *initrd_filename, const char *cpu_model)
{
    fm3_init(kernel_filename, cpu_model, &cqfm3_base_info);
}

static void cq_fm3_ex_init(ram_addr_t ram_size, const char *boot_device,
                           const char *kernel_filename, const char *kernel_cmdline,
                           const char *initrd_filename, const char *cpu_model)
{
