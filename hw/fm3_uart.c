...
static uint64_t fm3_uart_read(void *opaque, target_phys_addr_t offset,
                              unsigned size)
{
...
static void fm3_uart_write(void *opaque, target_phys_addr_t offset,
...
{
    Fm3UartState *s = container_of(d, Fm3UartState, busdev.qdev);
    Fm3UartChState *ch;
    int i;
                             ...
                             uint32_t ch_no)
{
    ...
    ch->ch_no = ch_no;
    ch->chr = qdev_init_chardev(&dev->qdev);
    if (ch->chr) {
...
{
    Fm3UartState *s = FROM_SYSBUS(Fm3UartState, dev);

    fm3_uart_ch_init(&s->ch[0], dev, 0);
    fm3_uart_ch_init(&s->ch[3], dev, 3);
    fm3_uart_ch_init(&s->ch[4], dev, 4);

    memory_region_init_io(&s->mmio, &fm3_uart_mem_ops, s, 
                          "fm3.uart", 0x1000);
...
static SysBusDeviceInfo fm3_uart_info = {
    .init = fm3_uart_init,
    .qdev.name  = "fm3.uart",
    .qdev.size  = sizeof(Fm3UartState),
    .qdev.reset = fm3_uart_reset,
    .qdev.props = (Property[]) {
...
{
    sysbus_register_withprop(&fm3_uart_info);
}
